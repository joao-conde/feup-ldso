...
describe('FacultySocialProjectController', () => {
  ...
  beforeEach(() => {
    ...
    //findById = socialRepo.findById as sinon.SinonStub;
    find = socialRepo.find as sinon.SinonStub;
    //deleteById = socialRepo.deleteById as sinon.SinonStub;
    //updateById = socialRepo.updateById as sinon.SinonStub;
  ...
  describe('GET /faculties/{language}/{name}/social-projects-short', () => {
    it('invokes correct find helper function', async () => {
      await controller.findFacultyProjectsShort('en', 'faup');

      sinon.assert.called(find);
    });

    it('retrieves the short version of social projects of a specific faculty (faup, english)', async () => {
      const model = [
        {
          title: 'Title from project 1',
          short_description: 'Short description from project 1',
      ...
      const result = await controller.findFacultyProjectsShort('en', 'faup');
      sinon.assert.called(find);
      expect(find.getCalls().length).to.equal(1); //1 call to 'find' to get all the projects' short version
      expect(result.length).to.equal(3); //3 projects
      expect(result).equal(model); //projects expected are returned
    });
  });
